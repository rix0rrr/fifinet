...
export interface IPipe<V, E> {
  step(graph: Graph<V, E>, gremlin: MaybeGremlin): Piperesult;
}
...
export class VertexSource<V, E> implements IPipe<V, E> {
  ...
  public step(graph: Graph<V, E>, gremlin: MaybeGremlin): Piperesult {
    if (!this.vertices) {
...
export class VertexFilterPipe<V, E> implements IPipe<V, E> {
  constructor(private readonly pattern: Partial<UserVertex<V>> | ((x: UserVertex<V>, g: MaybeGremlin) => boolean)) {
  }
...
export class AliasPipe<V, E> implements IPipe<V, E> {
  ...
  public step(graph: Graph<V, E>, gremlin: MaybeGremlin) {
    if(!gremlin) return 'pull'                            // query initialization
